...
abcd = ta.TablArray([[[[1, 10], [0, 1]]],  # air
...
db1 = ta.Assembly(abcd=abcd)

# cumulative abcd matrix
db1['cumabcd'] = ta.cummatmul(db1['abcd'], axis=0)
# get the last cumulative abcd matrix, then solve the eigens
eig_val, eig_vect = ta.linalg.eig(db1['cumabcd'].bcast[-1, :])
# pick one of the eigenvectors (the first one)
qv0 = eig_vect.cell[:, 1]
db1['qv0'] = qv0 / qv0[1]
qv = (ta.matmul(db1['cumabcd'], db1['qv0'])).cell
db1['qv'] = qv / qv[1]
inv_q = 1/(db1['qv'].cell[0])
db1['curve'] = inv_q.real
db1['w'] = ta.sqrt( (-0.001064/np.pi) / inv_q.imag)

opl_i = db1['abcd'].cell[0, 1]
db1['opl'] = ta.cumsum(opl_i.table, axis=0).cell
